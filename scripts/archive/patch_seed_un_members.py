...
for c in countries:
    ...
    if not c.get('unMember') or iso2 == 'VA':
        continue
    ...
    metadata = f'{{ source: "curated-un-member-state-seed", referenceOnly: true, unMemberState: true, studyDestinationCandidate: {is_candidate}, destinationReviewStatus: "UNREVIEWED", publicVisible: false, publicStatus: "DRAFT", note: "UN member state reference identity only. Admin may later mark this country as a study destination candidate; public visibility requires review and activation." }}'
